# Therapy-lake pages live on aistrangegame.com — for Ashrey's review

Standing ruling (via Claude Chat, 2026-09-23): therapy-lake pages get Ashrey's review before publishing. This list is every therapy-lake page **already live** on `main`, so he can review them. From now on, new therapy-lake pages wait in open PRs for his word; they are not merged by Claude Code.

**280 pages live.** 280 were generated in the register waves (each file reviewed by Opus before render; principle only, course text never reproduced, no source line naming the course). 0 were in the original drop.

How to use this: tick a page when it's fine; for anything to change or unpublish, note it in the last column. Claude Code will act on the notes.

Lake source: `asg-learning-register.json` (the register handoff). Walks (the two 30-day courses) are therapy-lake too; they are not live yet and will arrive in an open PR.
